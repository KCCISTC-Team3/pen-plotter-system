...
def process_and_save(
    ...
    cv2.imwrite(os.path.join(out_dir, "02_gaussian.png"), blur)
    cv2.imwrite(os.path.join(out_dir, "03_sobel_mag.png"), mag_u8)
    cv2.imwrite(os.path.join(out_dir, "04_canny.png"), edges)

    # Save per-pixel hex txt (0x00 or 0xFF per pixel)
    # This is a raw raster dump: length = H*W bytes
    edges_bytes = edges.tobytes()
    save_hex_txt_bytes(edges_bytes, os.path.join(out_dir, "05_canny_pixels_hex.txt"), mode=hex_mode)
    ...
    if save_packed_1bpp:
        ...
        save_hex_txt_bytes(packed, os.path.join(out_dir, "05_canny_packed_1bpp_hex.txt"), mode=hex_mode)

    print("Saved outputs to:", os.path.abspath(out_dir))
    print("PNG: 01_gray.png / 02_gaussian.png / 03_sobel_mag.png / 04_canny.png")
    print("HEX (per-pixel bytes): 05_canny_pixels_hex.txt")
    if save_packed_1bpp:
        print("HEX (packed 1bpp): 05_canny_packed_1bpp_hex.txt")
